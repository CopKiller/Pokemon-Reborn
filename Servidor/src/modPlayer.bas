...
Dim countOnline As Long
    ...
    frmServer.lbPlayers.List(Index - 1) = GetPlayerIP(Index) + " - " + GetPlayerLogin(Index)
...
Public Sub TryGivePlayerItem(ByVal Index As Long, ByVal ItemNum As Long, ByVal ItemVal As Long)
...
Public Sub GivePlayerPokemonExp(ByVal Index As Long, ByVal PokeSlot As Byte, ByVal Exp As Long)
    ...
    'Exp Rate
    Exp = Exp * frmServer.scrlExp.Value
...
Dim CountStat As Long, X As Byte
    ...
    If CountStat + Exp <= MAX_EV Then
        With PlayerPokemons(Index).Data(PokeSlot)
            If .Stat(evStat).EV + Exp <= 252 Then
                        ...
                        If IsPlaying(Index) Then
                            If TempPlayer(Index).UseChar > 0 Then
                                If .CustomScriptData2 > 0 Then
                                    GiveItem Index, .CustomScriptData, .CustomScriptData2
                                End If
                            End If
                        End If
                    End If
...
Dim ExpRollover As Long
    ...
    With Player(Index, TempPlayer(Index).UseChar)
        ...
        .CurExp = .CurExp + Exp * frmServer.scrlExp.Value
