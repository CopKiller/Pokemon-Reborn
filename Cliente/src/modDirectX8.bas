...
Public Tex_PokemonIcon_s() As Long
...
Public Count_PokemonIcon_s As Long
...
Private Sub CacheTextures()
    ...
    Do
        ...
        Case 28: TextureName = "incubator-window"
        Case Else: TextureName = i
        End Select
    ...
    i = 1 ' Shiny
    Do
        path = App.path & PokemonIcon_Path & i & "_icon_s" & GFX_EXT
        If FileCheck(path) Then
            Count_PokemonIcon_s = i
            ReDim Preserve Tex_PokemonIcon_s(1 To i)
            Tex_PokemonIcon_s(i) = SetTexturePath(path)
            i = i + 1
        Else
            i = 0
        End If
            ...
            If CanShowButton(i) Then
                ...
                Case ButtonEnum.Game_VirtualShop
                    If GUI(GuiEnum.GUI_VIRTUALSHOP).Visible Then
                        Button(i).State = ButtonState.StateClick
                    End If
                Case ButtonEnum.Game_Incubator
                    If GUI(GuiEnum.GUI_INCUBATOR).Visible Then
                        Button(i).State = ButtonState.StateClick
                    End If
            ...
            If GuiVisibleCount > 0 Then
                For i = 1 To GuiVisibleCount
                    If CanShowGui(GuiZOrder(i)) Then
                        ...
                        Case GuiEnum.GUI_INCUBATOR: DrawIncubator
...
Dim Sprite As Long
    ...
    With MapNpc(MapNpcNum)
        ...
        '//Render
        RenderTexture Tex_Character(Sprite), ConvertMapX(X), ConvertMapY(y), Anim * oWidth, rDir * oHeight, Width, Height, oWidth, oHeight, D3DColorARGB(.Opacity, 255, 255, 255)
    End With
End Sub
    ...
    With MapPokemon(PokemonIndex)
        ...
        If .IsShiny = YES Then
            If Sprite > 0 And Sprite <= Count_ShinyPokemon Then
                RenderTexture Tex_ShinyPokemon(Sprite), ConvertMapX(X), ConvertMapY(y), SpriteAnim * oWidth, oHeight, Width, Height, oWidth, oHeight, D3DColorARGB(.Opacity, 255, 255, 255)
            End If
        Else
            RenderTexture Tex_Pokemon(Sprite), ConvertMapX(X), ConvertMapY(y), SpriteAnim * oWidth, oHeight, Width, Height, oWidth, oHeight, D3DColorARGB(.Opacity, 255, 255, 255)
        End If
    End With
End Sub
...
Dim DrawAlpha As Byte
    ...
    With PlayerPokemon(Index)
        ...
        If .IsShiny = YES Then
            If Sprite > 0 And Sprite <= Count_ShinyPokemon Then
                RenderTexture Tex_ShinyPokemon(Sprite), ConvertMapX(X), ConvertMapY(y), SpriteAnim * oWidth, oHeight, Width, Height, oWidth, oHeight, D3DColorARGB(Player(Index).Opacity, 255, 255, 255)
            End If
        Else
            RenderTexture Tex_Pokemon(Sprite), ConvertMapX(X), ConvertMapY(y), SpriteAnim * oWidth, oHeight, Width, Height, oWidth, oHeight, D3DColorARGB(Player(Index).Opacity, 255, 255, 255)
        End If
    End With
End Sub
...
Dim SpritePos As Byte
    ...
    With MapNpcPokemon(Index)
        ...
        If .IsShiny = YES Then
            If Sprite > 0 And Sprite <= Count_ShinyPokemon Then
                RenderTexture Tex_ShinyPokemon(Sprite), ConvertMapX(X), ConvertMapY(y), SpriteAnim * oWidth, oHeight, Width, Height, oWidth, oHeight, D3DColorARGB(MapNpc(Index).Opacity, 255, 255, 255)
            End If
        Else
            RenderTexture Tex_Pokemon(Sprite), ConvertMapX(X), ConvertMapY(y), SpriteAnim * oWidth, oHeight, Width, Height, oWidth, oHeight, D3DColorARGB(MapNpc(Index).Opacity, 255, 255, 255)
        End If
    End With
End Sub
...
Dim X As Long, y As Long
    ...
    RenderTexture Tex_Surface(gSurfaceEnum.CompanyScreen), 0, 0, 0, 0, Screen_Width, Screen_Height, Width, Height
End Sub
...
Dim i As Long
    ...
    '//Render the company logo which present the game in the middle of the screen
    '//Get Size for quick access
    Width = GetPicWidth(Tex_Surface(gSurfaceEnum.TitleScreen)) / 2
    Height = GetPicHeight(Tex_Surface(gSurfaceEnum.TitleScreen)) / 2
    '//Set Location to center
    X = (Screen_Width / 2) - (Width / 2)
    y = (Screen_Height / 2) - (Height / 2)
    
    RenderTexture Tex_Surface(gSurfaceEnum.TitleScreen), X, y, 0, 0, Width, Height, Width * 2, Height * 2
End Sub
...
Dim Color As Long
    ...
    If Pokemon_HighIndex > 0 Then
        For i = 1 To Pokemon_HighIndex
            If Not CatchBall(i).InUsed Then
                If MapPokemon(i).Num > 0 And (InDuelTargetIndex = 0 Or (InDuelTargetIndex = i And InDuelTargetType = TARGET_TYPE_MAPPOKEMON)) Then
                    If MapPokemon(i).Map = Player(MyIndex).Map Then
    ...
    If Player_HighIndex > 0 Then
        For i = 1 To Player_HighIndex
            If IsPlaying(i) Then
                If Player(i).Map = Player(MyIndex).Map And (InDuelTargetIndex = 0 Or (InDuelTargetIndex = i And InDuelTargetType = TARGET_TYPE_PLAYER) Or i = MyIndex) Then
                    If Player(i).StealthMode = NO Then
                        If Player(i).CurHP < GetPlayerHP(Player(i).Level) Then
                            ...
                            X = ((Player(i).X * TILE_X) + Player(i).xOffset) - ((MaxWidth / 2) - (TILE_X / 4))
    ...
    For i = 1 To MAX_MAP_NPC
        If MapNpc(i).Num > 0 And (InDuelTargetIndex = 0 Or (InDuelTargetIndex = i And InDuelTargetType = TARGET_TYPE_NPC)) Then
...
Private Sub DrawHud()
    ...
    For i = 1 To MAX_PLAYER_POKEMON
        If PlayerPokemons(i).Num > 0 Then
            ...
            If Pokemon(PlayerPokemons(i).Num).Sprite > 0 And Pokemon(PlayerPokemons(i).Num).Sprite <= Count_PokemonIcon Then
                If PlayerPokemons(i).IsShiny = YES And Count_PokemonIcon_s >= Pokemon(PlayerPokemons(i).Num).Sprite Then
                    RenderTexture Tex_PokemonIcon_s(Pokemon(PlayerPokemons(i).Num).Sprite), X + 1, y + 1, initAnim * 32, 0, 32, 32, 32, 32, D3DColorARGB(Alpha, 255, 255, 255)
                Else
    ...
    For i = 1 To MAX_HOTBAR
        ...
        If Player(MyIndex).Hotbar(i).Num > 0 Then
            '//Draw Icon
            Sprite = Item(Player(MyIndex).Hotbar(i).Num).Sprite
    ...
    If PlayerPokemon(MyIndex).Num > 0 Then
        ...
        If Pokemon(PlayerPokemon(MyIndex).Num).Sprite > 0 And Pokemon(PlayerPokemon(MyIndex).Num).Sprite < Count_PokemonIcon Then
            If PlayerPokemon(MyIndex).IsShiny = YES And Count_PokemonIcon_s >= Pokemon(PlayerPokemon(MyIndex).Num).Sprite Then
                RenderTexture Tex_PokemonIcon_s(Pokemon(PlayerPokemon(MyIndex).Num).Sprite), 6, 3, MapAnim * 32, 0, 32, 32, 32, 32
            Else
            ...
            If i <> DragPokeSlot Then
                ...
                If PlayerPokemonStorage(PokemonCurSlot).Data(i).Num > 0 Then
                    ...
                    If Sprite > 0 And Sprite <= Count_PokemonIcon Then
                        ...
                        If PlayerPokemonStorage(PokemonCurSlot).Data(i).IsShiny = YES And Count_PokemonIcon_s >= Sprite Then
                            RenderTexture Tex_PokemonIcon_s(Sprite), DrawX, DrawY, MapAnim * 32, 0, 32, 32, 32, 32
                        Else
...
Private Sub DrawDragIcon()
    ...
    If DragPokeSlot > 0 Then
        If PlayerPokemonStorage(PokemonCurSlot).Data(DragPokeSlot).Num > 0 Then
            ...
            '//Draw Icon
            If Sprite > 0 And Sprite <= Count_PokemonIcon Then
                If PlayerPokemonStorage(PokemonCurSlot).Data(DragPokeSlot).IsShiny = YES And Count_PokemonIcon_s >= Sprite Then
                    RenderTexture Tex_PokemonIcon_s(Sprite), CursorX - (TILE_X / 2), CursorY - (TILE_Y / 2), MapAnim * TILE_X, 0, TILE_X, TILE_Y, TILE_X, TILE_Y
                Else
        ...
        For i = ShopAddY To ShopAddY + 8
            If i > 0 And i <= MAX_SHOP_ITEM Then
                ...
                If Shop(ShopNum).ShopItem(i).Num > 0 Then
                    ...
                    If Shop(ShopNum).ShopItem(i).SellValueType = 1 Then
                        IDValue = Item(Shop(ShopNum).ShopItem(i).SellValueId).Sprite
                    Else
                        IDValue = IDMoney
                    End If
            ...
            If YourTrade.Data(i).TradeType > 0 Then
                ...
                If YourTrade.Data(i).Num > 0 Then
                    ...
                    ElseIf YourTrade.Data(i).TradeType = 2 Then  '//Pokemon
                        ...
                        If Sprite > 0 And Sprite <= Count_PokemonIcon Then
                            If YourTrade.Data(i).IsShiny = YES And Count_PokemonIcon_s >= Sprite Then
                                RenderTexture Tex_PokemonIcon_s(Sprite), DrawX + 7, DrawY + 7, MapAnim * TILE_X, 0, TILE_X, TILE_Y, TILE_X, TILE_Y
                            Else
            ...
            If TheirTrade.Data(i).TradeType > 0 Then
                ...
                If TheirTrade.Data(i).Num > 0 Then
                    ...
                    ElseIf TheirTrade.Data(i).TradeType = 2 Then    '//Pokemon
                        ...
                        If Sprite > 0 And Sprite <= Count_PokemonIcon Then
                            If TheirTrade.Data(i).IsShiny = YES And Count_PokemonIcon_s >= Sprite Then
                                RenderTexture Tex_PokemonIcon_s(Sprite), DrawX + 7, DrawY + 7, MapAnim * TILE_X, 0, TILE_X, TILE_Y, TILE_X, TILE_Y
                            Else
                                RenderTexture Tex_PokemonIcon(Sprite), DrawX + 7, DrawY + 7, MapAnim * TILE_X, 0, TILE_X, TILE_Y, TILE_X, TILE_Y
                            End If
...
Public Sub DrawItemDesc()
    ...
    If Item(ItemID).IsCash = NO Then
        ...
        RenderTexture Tex_Item(IDMoney), StartX + ((150 * 0.5) - (GetTextWidth(Font_Default, ItemPrice) * 0.5)), StartY + 219 - 20, 0, 0, 20, 20, GetPicWidth(Tex_Item(IDMoney)), GetPicHeight(Tex_Item(IDMoney))
    Else
        ItemPrice = "Price: Non Sellable"
    End If

    RenderText Font_Default, ItemPrice, StartX + 6 + ((182 * 0.5) - (GetTextWidth(Font_Default, ItemPrice) * 0.5)), StartY + 219 - 20, White
